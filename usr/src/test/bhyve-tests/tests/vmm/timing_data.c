...
/*
 * VMM Timing Data interface tests
 *
...
/*
 * Test a valid VMM_DATA_READ of timing data
 */
static void
test_valid_read_timing_data(int vmfd, struct vdi_timing_info_v1 *timing_info)
{
...
/*
 * Test valid VMM_DATA_WRITE of timing data
 */
static void
test_valid_write_timing_data(int vmfd, struct vdi_timing_info_v1 *timing_info)
{
	struct vm_data_xfer xfer = {
		.vdx_class = VDC_VMM_TIMING,
		.vdx_version = 1,
...
/*
 * Test malformed VMM_DATA_READ timing data requests
 */
static void
test_invalid_read_timing_data(int vmfd)
{
	struct vdi_timing_info_v1 res;
	...
	if (ioctl(vmfd, VM_DATA_READ, &xfer) == 0) {
		errx(EXIT_FAILURE,
		    "invalid VMM_DATA_READ of timing info should fail");
	}
	error = errno;
	if (error != ENOSPC) {
		errx(EXIT_FAILURE, "test_invalid_read_timing_data: "
	...
	should_eq_u32("vdx_result_len", xfer.vdx_result_len,
	    sizeof (struct vdi_timing_info_v1));
}

/*
 * Test malformed VMM_DATA_WRITE timing data requests
 */
static void
test_invalid_write_timing_data(int vmfd, struct vdi_timing_info_v1 *src)
{
	/* invalid vdx_len */
	struct vm_data_xfer xfer = {
		.vdx_class = VDC_VMM_TIMING,
	...
	if (ioctl(vmfd, VM_DATA_WRITE, &xfer) == 0) {
		errx(EXIT_FAILURE,
		    "invalid VMM_DATA_WRITE of timing info should fail");
	}
	error = errno;
	if (error != ENOSPC) {
		errx(EXIT_FAILURE, "test_invalid_write_timing_data: "
		    "expected ENOSPC errno, got %d", error);
	}
	/* expected vdx_result_len should be communicated out */
	should_eq_u32("vdx_result_len", xfer.vdx_result_len,
 ...
 */
static void
test_invalid_freq(int vmfd, struct vdi_timing_info_v1 *src)
{
	/* guest frequency of 0 always invalid */
	struct vdi_timing_info_v1 invalid = {
		.vt_guest_freq = 0,
	...
	if (ioctl(vmfd, VM_DATA_WRITE, &xfer) == 0) {
		errx(EXIT_FAILURE,
		    "invalid VMM_DATA_WRITE of timing info (vt_guest_freq = 0) "
		    "should fail");
	}
	error = errno;
	if (error != EINVAL) {
 ...
 */
static void
test_invalid_freq_amd(int vmfd, struct vdi_timing_info_v1 *src)
{
	struct vdi_timing_info_v1 invalid = {
	...
	if (ioctl(vmfd, VM_DATA_WRITE, &xfer) == 0) {
		errx(EXIT_FAILURE, "invalid VMM_DATA_WRITE of timing info "
 ...
 */
static void
test_valid_freq_amd(int vmfd, struct vdi_timing_info_v1 *src)
{
	struct vdi_timing_info_v1 res;
	int error;

	/* minimum frequency */
	struct vdi_timing_info_v1 valid = {
	...
	if (ioctl(vmfd, VM_DATA_WRITE, &xfer) != 0) {
		error = errno;
		errx(EXIT_FAILURE, "valid VMM_DATA_WRITE of timing info "
		    "(min AMD guest frequency) should succeed, errno=%d",
		    error);
	}
	/* verify the frequency was changed */
	test_valid_read_timing_data(vmfd, &res);
 ...
 */
static void
test_invalid_freq_intel(int vmfd, struct vdi_timing_info_v1 *src)
{
	/*
...
/*
 * Test that host times from the future are not accepted
 */
static void
test_invalid_host_times(int vmfd, struct vdi_timing_info_v1 *src)
{
	struct vdi_timing_info_v1 invalid = {
		.vt_guest_freq = src->vt_guest_freq,
		.vt_guest_tsc = src->vt_guest_tsc,
	...
	error = errno;
	if (error != EINVAL) {
		errx(EXIT_FAILURE, "test_invalid_host_times (hrtime): "
		    "expected EINVAL errno, got %d", error);
	}

	/* hrestime + 500 seconds */
	invalid.vt_hrtime = src->vt_hrtime;
	invalid.vt_hres_sec += 500;
	if (ioctl(vmfd, VM_DATA_WRITE, &xfer) == 0) {
		errx(EXIT_FAILURE, "invalid VMM_DATA_WRITE of timing info "
		    "(hrestime in the future) should fail");
	}
	error = errno;
	if (error != EINVAL) {
		errx(EXIT_FAILURE, "test_invalid_host_times (hrestime): "
		    "expected EINVAL errno, got %d", error);
	}
}
 ...
 */
static void
test_valid_guest_tsc(int vmfd, struct vdi_timing_info_v1 *src)
{
	...
	test_valid_write_timing_data(vmfd, &valid);
...
/*
 * Test that a different boot_hrtime is accepted. There are no constraints on
 * what this value can be.
 */
static void
test_valid_boot_hrtime(int vmfd, struct vdi_timing_info_v1 *src)
{
	struct vdi_timing_info_v1 res;
	...
	/* repeat for boot_hrtime > 0 */
	valid.vt_boot_hrtime = 100000000000;
	test_valid_write_timing_data(vmfd, &valid);
	test_valid_read_timing_data(vmfd, &res);
	should_geq_i64("boot_hrtime", res.vt_boot_hrtime, valid.vt_boot_hrtime);
}
 ...
 */
static void
test_adjust(int vmfd, struct vdi_timing_info_v1 *src)
{
	struct vdi_timing_info_v1 res;
	test_valid_write_timing_data(vmfd, src);

	/* read it back */
	test_valid_read_timing_data(vmfd, &res);

	/*
	 * hrtime, hrestime, guest TSC, and boot_hrtime should all have moved
	 * forward
	 */
	...
	should_geq_i64("vt_boot_hrtime", res.vt_boot_hrtime,
	    src->vt_boot_hrtime);
...
main(int argc, char *argv[])
{
	 ...
	 */
	/* do a valid read */
	test_valid_read_timing_data(vmfd, &timing_info);

	/* malformed read request */
	test_invalid_read_timing_data(vmfd);
	 ...
	 * the CPU vendor of the system, or capability to scale a guest TSC,
	 * does not matter.
	 */

	/* try writing back the data from the read */
	test_valid_write_timing_data(vmfd, &timing_info);

	/* malformed write request */
	test_invalid_write_timing_data(vmfd, &timing_info);

	/* invalid host time requests */
	test_invalid_host_times(vmfd, &timing_info);

	/* invalid guest frequency requests */
	test_invalid_freq(vmfd, &timing_info);
	if (is_svm) {
		test_invalid_freq_amd(vmfd, &timing_info);
	} else {
		test_invalid_freq_intel(vmfd, &timing_info);
	}

	/* valid frequency scaling requests */
	if (is_svm) {
		test_valid_freq_amd(vmfd, &timing_info);
	}

	/* valid guest TSC values */
	test_valid_guest_tsc(vmfd, &timing_info);

	/* valid boot_hrtime values */
	test_valid_boot_hrtime(vmfd, &timing_info);

	/* observe that host times and guest data are updated after a write */
	test_adjust(vmfd, &timing_info);
