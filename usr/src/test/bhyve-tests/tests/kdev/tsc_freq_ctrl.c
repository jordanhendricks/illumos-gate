...
do_freq_test(uint64_t guest_freq, uint8_t per_sec, uint8_t seconds,
    const int vmfd, struct vmctx *ctx, struct vdi_timing_info_v1 *src)
{
...
main(int argc, char *argv[])
{
	...
	uint64_t host_freq = timing_info.vt_guest_freq;
	...
	do_freq_test(guest_freq, per_sec, seconds, vmfd, ctx, &timing_info);
